...
evalute the robustness of input word embeddings for language models.
...
class Inference:
        ...
        for seq in sequence:
            ...
            try:
                if generate_mode_test == "greedy":
                    ...
                    outputs=self.gen_virtualEmbedReSend(input_ids,eps,mask_p)
    ...
    def gen_virtualEmbedReSend(self,prefix_ids,eps,mask_p):
        """
        Embedding resend style sentence generation.
        """
        # print(">>> USING EMBEDRESEND GENERATION.")
...
def main_real():
    ...
    parser.add_argument("--mask_p",default=0.25,type=float)
    parser.add_argument("--eps",default=0.25,type=float)
    parser.add_argument("--cuda_num",default="cpu",type=str)
    ...
    newseqls=infermodel.inference(seqls,generate_mode_test="greedy",
                                  eps=args.eps,mask_p=args.mask_p)
