...
Inference with constant lengths.
...
class Inference:
    ...
    def evaluate2(self,hyps,refs):
        ...
        return res
    def pade(self,embed,pad_id,seq_ls):
        bs,sl,d=embed.shape
        pad_tensor=torch.ones((bs,max(0,seq_ls-sl)),dtype=torch.long)*pad_id
        pad_tensor=pad_tensor.to(self.device)
        pad_embedds=self.embedds(pad_tensor).to(self.device)
        newembed=torch.cat([embed,pad_embedds],dim=1)
        return newembed
    ...
    def gen_embedResend(self,prefix_ids):
        ...
        embeddings=self.pade(embeddings,self.eos_token_id,128)
        ...
        with torch.no_grad():
            for _ in range(gen_len):
                ...
                next_token_logits=output.logits[0,sl-1,:]
                ...
                if self.only_decoder:
                    newem=output.hidden_states[-1][:,sl-1,:]
                    ...
                    newem=output.decoder_hidden_states[-1][:,sl-1,:]
                    decoder_input_embedds[:,sl,:]=newem
